...
class MDFFundController extends Controller
{
    ...
    //
    // Column mapping:
    //   col (3) past obligation → pastPlan    snapshot.obligation_amount  [EDITABLE]
    //   col (4) sem1 actual     → currentPlan snapshot.sem1_actual        [EDITABLE]
    //   col (5) sem2            → currentPlan snapshot.sem2_actual        [auto-computed, displayed]
    //   col (6) cur total       → currentPlan snapshot.total_amount       [READ-ONLY]
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $result = $categories->map(function (MdfCategory $cat) use (
            ...
            $items = $cat->items->map(function (MdfItem $item) use (
                ...
                if ($item->obligation_id && $item->debt_type) {
                    ...
                    $pastObligation = $pay($item->obligation_id, $pastPlan?->budget_plan_id, $dueField);
                    $curTotal       = $pay($item->obligation_id, $currentPlan?->budget_plan_id, $dueField);
                    $curSem1        = $pay($item->obligation_id, $currentPlan?->budget_plan_id, $s1Field);
                    $curSem2        = max(0, $curTotal - $curSem1);
                    $proposed       = $pay($item->obligation_id, $activePlan->budget_plan_id, $dueField);
                    ...
                    return [
                        ...
                        'past_obligation'     => $pastObligation, // derived from debt_payments for debt rows
                        'cur_sem1'            => $curSem1,
                        'cur_sem2'            => $curSem2,
                        'cur_total'           => $curTotal,
                        'proposed'            => $proposed,
                ...
                $curTotal       = (float) ($currentSnap?->total_amount   ?? 0);
                $curSem1        = (float) ($currentSnap?->sem1_actual    ?? 0);
                // Use stored sem2_actual; fall back to derived if still 0 (pre-migration rows)
        ...
        return response()->json([
            ...
            'past_plan_missing' => $pastPlan === null, // ← frontend uses this to warn user
            'categories'        => $result,
            'grand_totals'      => $grandTotals,
        ]);
    }
    ...
    // POST /api/mdf-funds/save-proposed
    //
    // Save col (7) proposed → active plan snapshot.total_amount
    // Body: { item_id, budget_plan_id, total_amount }
    // ══════════════════════════════════════════════════════════════════════════
    ...
    // POST /api/mdf-funds/save-sem1
    //
    // Save col (4) sem1 → current year plan snapshot.sem1_actual.
    // Also auto-computes and stores sem2_actual = total_amount - sem1.
    // Body: { item_id, budget_plan_id, sem1_actual }
    // ══════════════════════════════════════════════════════════════════════════
    ...
    public function saveSem1(Request $request): JsonResponse
    {
        ...
        // Clamp sem1 to total_amount
        $clamped           = min(max((float) $v['sem1_actual'], 0), (float) $snap->total_amount);
        $snap->sem1_actual = $clamped;
        $snap->sem2_actual = max(0.0, (float) $snap->total_amount - $clamped); // ← store sem2
        $snap->save();
    ...
    //
    // Save col (3) past obligation → past year plan snapshot.obligation_amount.
    //
    // Rules (mirrors IncomeFundController):
    //   - Past plan must exist. If not → 422 with a clear message.
    //   - Saves ONLY to the past plan's snapshot, not the active plan.
    //   - Debt rows are read-only (obligation_amount derived from debt_payments).
    //
    // Body: { item_id, past_plan_id, obligation_amount }
    // ══════════════════════════════════════════════════════════════════════════
    ...
    public function saveObligation(Request $request): JsonResponse
    {
        $v = $request->validate([
            'item_id'           => 'required|integer|exists:mdf_items,item_id',
            'past_plan_id'      => 'required|integer',          // validated below
        ...
        // Verify the past plan actually exists
