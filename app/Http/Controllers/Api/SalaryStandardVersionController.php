...
class SalaryStandardVersionController extends BaseMasterCrudController
{
    ...
    public function storeWithSteps(Request $request)
    {
        ...
        $validated = $request->validate([
            'lbc_reference'          => ['required', 'string', 'max:100'],
            'tranche'                => ['required', Rule::in(['1st Tranche', '2nd Tranche', '3rd Tranche', '4th Tranche'])],
            'income_class'           => ['required', 'string', 'max:50'],
            'effective_year'         => ['required', 'date'],
            'is_active'              => ['boolean'],
            'steps'                  => ['required', 'array', 'min:1'],
        ...
        DB::transaction(function () use ($validated) {
            ...
            $steps = collect($validated['steps'])->map(fn ($item) => [
                'salary_standard_version_id' => $version->salary_standard_version_id,
                'salary_grade' => $item['salary_grade'],
                'step'         => $item['step'],
                'salary'       => $item['salary'],
    ...
    /**
     * Update an existing version's metadata AND replace all its salary steps
     * in a single atomic transaction.
     *
     * PUT /salary-standard-versions/{version}/with-steps
     */
    public function updateWithSteps(Request $request, SalaryStandardVersion $version)
    {
        $this->authorize('update', $version);
        ...
        DB::transaction(function () use ($validated, $version) {
            // If this version is being set active, deactivate all others first
            if (!empty($validated['is_active'])) {
                SalaryStandardVersion::where('is_active', true)
                    ->where('salary_standard_version_id', '!=', $version->salary_standard_version_id)
                    ->update(['is_active' => false]);
            ...
            $version->update([
                ...
                'income_class'   => $validated['income_class'],
                'effective_year' => $validated['effective_year'],
                'is_active'      => $validated['is_active'] ?? $version->is_active,
            ]);

            // Replace all steps atomically
            SalaryGradeStep::where('salary_standard_version_id', $version->salary_standard_version_id)
                ->delete();
        ...
        return $this->success([
            'message' => 'Salary version updated successfully',
            'version' => $version->fresh(['steps']),
        ]);
    }
