...
import { BudgetPlan, DepartmentBudgetPlan, Department } from "../../types/api";
import { cn } from "@/src/lib/utils";
import { BanknotesIcon } from "@heroicons/react/24/outline";
import { useDepartmentBudgetPlans, useDepartments } from "../../hooks/useDashboardQueries";
...
function computeBreakdown(
  deptPlans: DepartmentBudgetPlan[],
  departments: Department[],
): Breakdown {
  const deptMap = new Map<number, Department>(departments.map(d => [d.dept_id, d]));

  // Only general fund depts (not special accounts)
  const gfDps = deptPlans.filter(dp => {
    const d = deptMap.get(dp.dept_id);
    // If dept info not loaded yet, include it (safe default)
