...
export async function fetchFund(source: string): Promise<FundData> {
  try {
    ...
    // ANY error (404, 500, network) → return zeros, never throw
    return EMPTY_FUND;
  }
}
...
async function fetchDeptExpenditures(
  ...
  try {
    const [dpItemsRes, aipRes] = await Promise.allSettled([
      API.get('/department-budget-plans', { params: { budget_plan_id: activePlan.budget_plan_id } }),
...
export function useAllFunds() {
  return useQuery<AllFundsData>({
    ...
    queryFn:  async () => {
      // allSettled — one 404 never blocks the others
