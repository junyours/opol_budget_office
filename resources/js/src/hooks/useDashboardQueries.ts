...
// // ─── Pure computation helpers (no API calls) ──────────────────────────────────

// /**
//  * Compute per-dept expenditure totals from already-fetched data.
//  * Called inside React Query queryFn — no raw API.get here.
//  */
// function computeDeptExpenditures(
//   deptBudgetPlans: DepartmentBudgetPlan[],
//   aipPrograms: any[],
//   departments: Department[],
// ): DeptExpenditure[] {
//   const deptMap = new Map<number, Department>(departments.map(d => [d.dept_id, d]));
...
// /**
//  * Compute special account expenditures (SH / OCC / PM) from already-fetched data.
//  */
// function computeSpecialAccountExpenditures(
//   deptBudgetPlans: DepartmentBudgetPlan[],
//   aipPrograms: any[],
//   departments: Department[],
// ): SpecialAccountExpenditures {
//   const specialDepts = departments.filter(d => d.dept_category_id === SPECIAL_CAT_ID);
//   if (specialDepts.length === 0) return EMPTY_SPECIAL_EXP;
...
//   budgetPlans:              ['budget-plans']                                         as const,
//   deptBudgetPlans:          (planId: number) => ['dept-budget-plans', planId]        as const,
//   // NOTE: key must match useBudgetTotals and useAipProgramData exactly
...
// /** Shared AIP programs query — same key as useBudgetTotals & useAipProgramData */
...
// /**
//  * Compute dept expenditures from the CACHED dept-budget-plans and aip-programs
//  * queries — no extra API calls fired.
//  */
// export function useDeptExpenditures(
//   budgetPlanId: number | undefined,
//   departments: Department[]
// ) {
//   // Re-use cached data — these queries are already in-flight or settled
//   const { data: deptPlans = [] }  = useDepartmentBudgetPlans(budgetPlanId);
//   const { data: aipPrograms = [] } = useAipPrograms(budgetPlanId);

//   return useQuery<DeptExpenditure[]>({
//     queryKey: queryKeys.deptExpenditures(budgetPlanId!),
//     queryFn:  () => computeDeptExpenditures(deptPlans, aipPrograms, departments),
//     // Only run after both upstream queries have data
//     enabled:  !!budgetPlanId && departments.length > 0 && deptPlans.length > 0,
//     // Invalidate automatically when the upstream data changes via normal cache TTL
//     staleTime: 0,
//     retry: false,
//   });
// }
...
// /**
//  * Compute special-account expenditures (SH / OCC / PM) from cached data.
//  */
// export function useSpecialAccountExpenditures(
//   budgetPlanId: number | undefined,
//   departments: Department[]
...
//   return useQuery<SpecialAccountExpenditures>({
//     queryKey: queryKeys.specialAccountExp(budgetPlanId!),
//     queryFn:  () => computeSpecialAccountExpenditures(deptPlans, aipPrograms, departments),
//     enabled:  !!budgetPlanId && departments.length > 0 && deptPlans.length > 0,
//     staleTime: 0,
//     retry: false,
//   });
