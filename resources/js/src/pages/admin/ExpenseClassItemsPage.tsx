...
const ExpenseClassItemsPage: React.FC = () => {
  ...
  // ── Sort ───────────────────────────────────────────────────────────────────
  const [sortField, setSortField] = useState<"name" | "acc_code" | "class">("name");
  ...
  const isFiltered  = filterClass !== ALL_CLASSIFICATIONS || filterStatus !== ALL_STATUS;
  const isSearching = debouncedSearch.trim().length > 0;
  const activeClass = classifications.find((c) => c.expense_class_id.toString() === filterClass);
  ...
  const openCreate = () => {
    setEditingItem(null);
    setForm({ expense_class_id: "", expense_class_item_name: "", expense_class_item_acc_code: "", is_active: true });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: Record<string, any> = {
        ...
        expense_class_item_acc_code: form.expense_class_item_acc_code.trim() || null,
        is_active:                   form.is_active,
      };
      if (form.expense_class_id) payload.expense_class_id = Number(form.expense_class_id);

      if (editingItem) {
        await API.put(`/expense-class-items/${editingItem.expense_class_item_id}`, payload);
  ...
  const SortIcon = ({ field }: { field: typeof sortField }) => (
    <span className={cn(
      ...
      sortField === field ? "text-gray-700" : "text-gray-300"
    )}>
    ...
    <div className="p-6">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <Select value={filterClass} onValueChange={handleFilterClassChange}>
          ...
          <SelectContent>
            <SelectItem value={ALL_CLASSIFICATIONS} className="text-xs">All classifications</SelectItem>
            {classifications.map((c) => (
              <SelectItem key={c.expense_class_id} value={c.expense_class_id.toString()} className="text-xs">
                {c.expense_class_name}{c.abbreviation ? ` (${c.abbreviation})` : ""}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
          ...
          <div className="flex items-center gap-1.5">
            {filterClass !== ALL_CLASSIFICATIONS && (
              <span className="flex items-center gap-1 text-[11px] text-gray-600 bg-gray-100 border border-gray-200 rounded-full px-2.5 py-1">
                ...
                <button onClick={() => { setFilterClass(ALL_CLASSIFICATIONS); setPage(1); }} className="text-gray-400 hover:text-gray-700 ml-0.5"><XMarkIcon className="w-3 h-3" /></button>
              </span>
            )}
            {filterStatus !== ALL_STATUS && (
              <span className="flex items-center gap-1 text-[11px] text-gray-600 bg-gray-100 border border-gray-200 rounded-full px-2.5 py-1">
                {filterStatus === "active" ? "Active" : "Inactive"}
                <button onClick={() => { setFilterStatus(ALL_STATUS); setPage(1); }} className="text-gray-400 hover:text-gray-700 ml-0.5"><XMarkIcon className="w-3 h-3" /></button>
              </span>
            )}
            <button onClick={clearFilters} className="text-[11px] text-gray-400 hover:text-gray-700 underline underline-offset-2">
              Clear all
            </button>
          </div>
        )}
        ...
        <span className="text-[11px] text-gray-400 ml-auto">
          {isFiltered || isSearching ? (
            ...
            <><span className="font-medium text-gray-600">{items.length}</span> item{items.length !== 1 ? "s" : ""}</>
          )}
        </span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        {filtered.length === 0 ? (
          <div className="text-center py-14 text-gray-400 text-sm">
            {isSearching ? (
              <>No items match <span className="font-medium text-gray-600">"{debouncedSearch}"</span>.{" "}
              ...
              <>No items match the selected filters.{" "}
                <button onClick={clearFilters} className="text-gray-600 underline underline-offset-2 font-medium hover:text-gray-900">Clear filters</button></>
            ) : (
            ...
            <table className="w-full text-[12px] border-collapse">
              <thead>
                <tr>
                  ...
                  >
                    Acc. Code <SortIcon field="acc_code" />
                  </th>
                  <th
                  ...
                  >
                    Classification <SortIcon field="class" />
                  </th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-500 font-mono text-[11px]">
                      {item.expense_class_item_acc_code
                        ? isSearching
                          ? highlightMatch(item.expense_class_item_acc_code, debouncedSearch)
                          : item.expense_class_item_acc_code
                      ...
                      {item.classification ? (
                        <span className="inline-flex items-center gap-1">
                          <span className="text-gray-700 font-medium">
                            {isSearching
                              ? highlightMatch(item.classification.expense_class_name, debouncedSearch)
        ...
        <DialogContent className="max-w-lg rounded-2xl border-gray-200 gap-0 p-0 overflow-hidden">
          <DialogHeader className="px-6 pt-5 pb-4 border-b border-gray-100">
            ...
            <DialogDescription className="text-xs text-gray-400 mt-0.5">
              {editingItem ? "Update the expense item details." : "Fill in the details for the new expense item."}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="px-6 py-5 space-y-4">
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setForm((p) => ({ ...p, expense_class_id: v }))}
              >
                ...
                <SelectContent>
                  {classifications.map((c) => (
                    <SelectItem key={c.expense_class_id} value={c.expense_class_id.toString()}>
                      {c.expense_class_name}{c.abbreviation ? ` (${c.abbreviation})` : ""}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => setForm((p) => ({ ...p, expense_class_item_name: e.target.value }))}
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => setForm((p) => ({ ...p, expense_class_item_acc_code: e.target.value }))}
        ...
        <AlertDialogContent className="rounded-2xl max-w-sm border-gray-200">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[15px] font-semibold">
              {deactivateTarget?.is_active ? "Deactivate item?" : "Activate item?"}
            </AlertDialogTitle>
            <AlertDialogDescription className="text-sm text-gray-500">
              <span className="font-medium text-gray-700">{deactivateTarget?.expense_class_item_name}</span>{" "}
              will be {deactivateTarget?.is_active ? "deactivated and hidden from budget plan forms." : "activated and available for use."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction asChild>
              <Button
                size="sm"
                className={cn("h-8 text-xs", deactivateTarget?.is_active ? "bg-amber-600 hover:bg-amber-700" : "bg-emerald-600 hover:bg-emerald-700")}
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => { if (!o) setDeleteTarget(null); }}>
        <AlertDialogContent className="rounded-2xl max-w-sm border-gray-200">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[15px] font-semibold">Delete expense item?</AlertDialogTitle>
            <AlertDialogDescription className="text-sm text-gray-500">
              <span className="font-medium text-gray-700">{deleteTarget?.expense_class_item_name}</span> will
