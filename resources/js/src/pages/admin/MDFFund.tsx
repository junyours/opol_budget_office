...
//               </td>
//               <td className="border-r border-l px-2 py-2 bg-green-50/30 border-green-100">
...
//                 <Skeleton className="h-3 ml-auto rounded bg-blue-100 w-14" />
//               </td>
//               <td className="border-r px-3 py-2.5 bg-blue-50/30 border-blue-100">
//                 <Skeleton className="h-3 ml-auto rounded bg-blue-100 w-16" />
//               </td>
...
//   categoryName: string;
//   activePlanId: number;
//   /** item_ids already rendered in this category's table rows */
//   existingItemIds: Set<number>;
//   /** lower-cased item names already in this category (for new-item dupe check) */
...
//       toast.error(
//         `"${name.trim()}" already exists in this table. Use "Search existing items" to re-add it, or choose a different name.`,
//         { duration: 6000 }
//       );
//       return;
//     }
...
//   // Live duplicate-name warning while typing
//   const nameIsDuplicate = name.trim().length > 0 && existingItemNames.has(name.trim().toLowerCase());
...
//                   className="h-9 text-sm pl-9" autoFocus />
//               </div>
//             </div>
//             <div className="overflow-y-auto flex-1 px-3 py-2">
//               {fetching && (
//                 <div className="flex items-center justify-center py-6 text-gray-400 text-sm gap-2">
//                   <span className="w-3.5 h-3.5 border-2 border-gray-200 border-t-gray-400 rounded-full animate-spin" />Searching…
//                 </div>
//               )}
//               {!fetching && search && results.length === 0 && (
//                 <p className="text-center py-6 text-[12px] text-gray-400">No items found for "{search}"</p>
//               )}
//               {!fetching && !search && (
//                 <p className="text-center py-6 text-[12px] text-gray-400">Type to search existing MDF items</p>
//               )}
//               {!fetching && results.map(item => {
//                 const alreadyAdded = existingItemIds.has(item.item_id);
//                 return (
//                   <button key={item.item_id}
//                     onClick={() => !alreadyAdded && handleSelectExisting(item)}
//                     disabled={saving || alreadyAdded}
//                     title={alreadyAdded ? "Already added to this table" : undefined}
//                     className={cn(
...
//                     )}>
//                     <div>
//                       <p className={cn("text-[12px] font-medium", alreadyAdded ? "text-gray-400" : "text-gray-800")}>
//                         {item.name}
//                       </p>
//                       {item.category_name && (
//                         <p className="text-[10px] text-gray-400 mt-0.5">{item.category_name}</p>
//                       )}
//                     </div>
//                     <div className="flex items-center gap-2 flex-shrink-0 ml-2">
//                       {alreadyAdded && (
//                         <span className="text-[10px] text-gray-400 bg-gray-100 border border-gray-200 rounded px-1.5 py-0.5 font-medium">
//                           Added
//                         </span>
//                       )}
//                       {item.account_code && (
...
//                   className={cn(
//                     "h-9 text-sm",
//                     nameIsDuplicate && "border-red-300 focus-visible:ring-red-300"
//                   )}
//                   autoFocus
//                   onKeyDown={e => e.key === "Enter" && !nameIsDuplicate && handleCreateNew()}
//                 />
//                 {nameIsDuplicate && (
//                   <p className="text-[11px] text-red-500 flex items-center gap-1 mt-1">
//                     <span>⚠</span>
//                     This item already exists in the table. Use "Search existing items" to re-add it.
//                   </p>
//                 )}
//               </div>
//               <div className="space-y-1.5">
...
//                   placeholder="e.g. 8-01-01" className="h-9 text-sm" />
//               </div>
//             </div>
...
//                 onClick={() => setMode("choose")} disabled={saving}>← Back</Button>
//               <Button size="sm" onClick={handleCreateNew}
//                 disabled={saving || !name.trim() || nameIsDuplicate}
...
//   // Last-saved values used to skip no-op saves
...
//     if (clamped !== rawSem1) {
//       setSem1Edits(prev => ({ ...prev, [itemId]: toStr(clamped) }));
//     }
//     const last = savedSem1.current.get(itemId);
//     if (last === clamped) return;
//     const promise = API.post("/mdf-funds/save-sem1", {
//       item_id: itemId, budget_plan_id: y.current_plan_id, sem1_actual: clamped,
//     }).then((res) => {
...
//           item.item_id === itemId
//             ? { ...item, cur_sem1: clamped, cur_sem2: sem2 }
//             : item
//         ),
//       })));
//     });
...
//   const handleObligationBlur = useCallback((itemId: number) => {
//     const y = yearsRef.current;
//     if (!y?.past_plan_id) {
//       toast.error(
...
//         { duration: 7000 }
//       );
//       const last = savedObligation.current.get(itemId) ?? 0;
//       setObligationEdits(prev => ({ ...prev, [itemId]: toStr(last) }));
//       return;
//     }
//     const cur  = parseNum(obligationRef.current[itemId] ?? "");
//     const last = savedObligation.current.get(itemId);
//     if (last === cur) return;
//     const promise = API.post("/mdf-funds/save-obligation", {
//       item_id: itemId, past_plan_id: y.past_plan_id, obligation_amount: cur,
//     }).then(() => savedObligation.current.set(itemId, cur));
//     toast.promise(promise, {
//       loading: "Saving…", success: "Saved",
//       error: (e: any) => `Save failed: ${e?.response?.data?.message ?? e?.message ?? "Unknown error"}`,
//     });
...
//               <thead>
//                 <tr>
//                   <th className={cn(thBase, "bg-white border-gray-200 text-left text-gray-700")}>
//                     Object of Expenditure
//                   </th>
//                   <th className={cn(thBase, "bg-white border-gray-200 text-center text-gray-500")}>
//                     Acct. Code
//                   </th>
//                   <th className={cn(thBase, "text-right", C_PAST_TH)}>
//                     {years?.past ?? "–"}
//                     <span className="block text-[9px] font-normal normal-case tracking-normal text-green-500">
//                       Past Yr Obligation
//                     </span>
//                   </th>
...
//                     <span className="block text-[9px] font-normal normal-case tracking-normal text-orange-500">
//                       Budget Year
//                     </span>
//                   </th>
//                 </tr>
...
//                       const oblEditable = !isDebt && !pastPlanMissing;
...
//                                 onBlur={() => handleObligationBlur(item.item_id)}
//                               />
//                             ) : (
//                               <span className={cn(
...
//           <span className="text-green-600 font-semibold">Green</span> = Past year obligation · editable · saves to {years?.past} plan
//         </span>
//         <span className="flex items-center gap-1.5">
//           <span className="w-2.5 h-2.5 rounded-sm bg-blue-50 border border-blue-200 inline-block" />
//           <span className="text-blue-600 font-semibold">Blue</span> = Current year · Sem 1 editable · Sem 2 auto-computed &amp; stored
//         </span>
//         <span className="flex items-center gap-1.5">
...
//         <span className="flex items-center gap-1.5">
//           <span className="w-2.5 h-2.5 rounded-sm bg-blue-50 border border-blue-200 inline-block" />
//           Debt-servicing rows — auto-filled from Form 5 (read-only)
//         </span>
//       </div>

//       {/* Add Item Dialog — builds existence sets from the target category */}
...
//         const existingItemNames = new Set(cat?.items.map(i => i.name.toLowerCase())   ?? []);
//         return (
//           <AddItemDialog
//             open={addDialog.open}
...
//               and all its snapshot data will be permanently removed.
...
interface AddItemDialogProps {
  ...
  activePlanId: number;
  /**
   * item_ids already visible in this category's table rows.
   * Used ONLY in search mode to disable items that are already added.
   */
  existingItemIds: Set<number>;
  onClose: () => void;
  onCreated: (item: MdfItem) => void;
...
function AddItemDialog({
  ...
  // ── Search mode ────────────────────────────────────────────────────────────
  // Load all items in this category on mount; filter client-side by search text.
  const [allCategoryItems, setAllCategoryItems] = useState<ExistingMdfItem[]>([]);
  const [search,           setSearch]           = useState("");
  const [loadingItems,     setLoadingItems]     = useState(false);

  // ── New item: live DB duplicate-name check ─────────────────────────────────
  const [nameCheckState, setNameCheckState] = useState<"idle" | "checking" | "available" | "taken">("idle");
  const nameCheckTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const reset = () => {
    setMode("choose");
    setName(""); setCode(""); setSearch("");
    setAllCategoryItems([]); setLoadingItems(false);
    setNameCheckState("idle");
  };

  // Load all items for this category when entering search mode
  useEffect(() => {
    if (mode !== "search") return;
  ...
  // Client-side filter — empty search shows all items
  ...
  // Debounced DB name check while typing in "new" mode
  useEffect(() => {
    if (mode !== "new") return;
  ...
  try {
    const res = await API.get<{ taken: boolean }>(
      `/mdf-items?check_name=${encodeURIComponent(trimmed)}`
    );
  ...
  // ── Add existing item from search list to the table ────────────────────────
  const handleSelectExisting = async (existing: ExistingMdfItem) => {
    if (existingItemIds.has(existing.item_id)) return; // guard: already in table
    setSaving(true);
    try {
      const snapRes = await API.post<{ data: MdfSnapshotResponse }>("/mdf-funds/save-proposed", {
        item_id: existing.item_id, budget_plan_id: activePlanId, total_amount: 0,
  ...
  // ── Create a brand-new mdf_item record in the DB, then add to table ────────
  const handleCreateNew = async () => {
    const trimmed = name.trim();
    if (!trimmed) return;
    if (nameCheckState === "taken") {
      toast.error(
        `"${trimmed}" already exists in the database. Use "Search existing items" to add it to the table.`,
        { duration: 6000 }
      );
      return;
    }
    ...
    try {
      ...
      const newItem = itemRes.data.data;
      await API.post("/mdf-funds/save-proposed", {
        item_id: newItem.item_id, budget_plan_id: activePlanId, total_amount: 0,
      });
      onCreated({
        ...newItem, has_prior_data: true,
        past_obligation: 0, cur_sem1: 0, cur_sem2: 0, cur_total: 0, proposed: 0,
      ...
      // Handle server-side uniqueness violation
      ...
      <DialogContent className="max-w-md rounded-2xl border-gray-200 gap-0 p-0 overflow-hidden" aria-describedby={undefined}>
            ...
            {[
              {
                ...
                sub: "Pick an item already in the database and add it to this table",
              },
              {
                m: "new" as const,
                icon: PlusIcon,
                title: "Create new item",
                sub: "Save a brand-new line item to the database and add it here",
              },
            ].map(({ m, icon: Icon, title, sub }) => (
              <button key={m} onClick={() => setMode(m)}
                className="w-full flex items-start gap-3 px-4 py-3.5 rounded-xl border border-gray-200 hover:border-gray-400 hover:bg-gray-50 transition-all text-left group">
        ...
        {mode === "search" && (
          <div className="flex flex-col" style={{ maxHeight: 460 }}>
            ...
            {/* Item list — all items in category by default */}
            <div className="overflow-y-auto flex-1 px-3 py-2">
              {loadingItems && (
                  ...
                  >
                    <div>
                      <p className={cn(
                        ...
                        alreadyInTable ? "text-gray-400" : "text-gray-800"
                      )}>
                        {item.name}
                      </p>
                      {item.category_name && (
        ...
        {mode === "new" && (
          <>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Input
                    ...
                    autoFocus
                    onKeyDown={e => e.key === "Enter" && canSubmit && handleCreateNew()}
                  />
                  {/* Spinner / check / cross inside the input */}
                ...
                {nameIsTaken && (
                  <p className="text-[11px] text-red-500 mt-1">
                    ...
                    <button
                      ...
                      onClick={() => { setMode("search"); setSearch(name.trim()); }}
                    >
                      Search existing items
                    </button>{" "}
                    to add it to the table instead.
                  </p>
                )}
                {nameCheckState === "available" && (
                  <p className="text-[11px] text-green-600 mt-1">Name is available.</p>
                )}
              </div>
            ...
            <DialogFooter className="px-6 py-4 border-t border-gray-100 gap-2">
              <Button variant="ghost" size="sm" className="h-8 text-xs text-gray-500"
                onClick={() => setMode("choose")} disabled={saving}>
                ← Back
              </Button>
              <Button
                size="sm"
...
export default function MDFFund() {
  ...
  const handleSem1Blur = useCallback((itemId: number) => {
    ...
    const promise = API.post("/mdf-funds/save-sem1", {
      ...
      setCategories(prev => prev.map(cat => ({
        ...cat, items: cat.items.map(item =>
          item.item_id === itemId ? { ...item, cur_sem1: clamped, cur_sem2: sem2 } : item
        ),
      })));
    });
    toast.promise(promise, {
...
const handleConfirmDelete = async () => {
  ...
  try {
    // ✅ Delete the SNAPSHOT for this budget plan, not the mdf_item master record
  ...
  return (
    <div className="p-6">
      ...
      {/* Add Item Dialog — passes only the IDs already in the target category's table */}
      {addDialog && activePlanId && (() => {
        const cat = computedCategories.find(c => c.category_id === addDialog.categoryId);
        const existingItemIds = new Set(cat?.items.map(i => i.item_id) ?? []);
        return (
