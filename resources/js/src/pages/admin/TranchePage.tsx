...
const TranchePage: React.FC = () => {
  ...
  const [editingVersion, setEditingVersion] = useState<VersionWithSteps | null>(null);
  ...
  const openEdit = (version: VersionWithSteps) => {
  ...
  // Build the version fields pre-populated for the editing version
  ...
  useEffect(() => {
    if (editingVersion) {
      setEditFields({
        lbc_reference: editingVersion.lbc_reference ?? "",
        tranche: editingVersion.tranche ?? "1st Tranche",
        income_class: editingVersion.income_class ?? "",
        effective_date: editingVersion.effective_year
          ? new Date(editingVersion.effective_year)
          : new Date(),
        is_active: editingVersion.is_active ?? false,
      });
    }
  }, [editingVersion]);

  // Convert the existing steps into the flat row format ExcelUploadModal expects
  const buildEditPreviewData = (version: VersionWithSteps) => {
    const matrix = getMatrix(version);
    return matrix.map(grade => ({
      salary_grade: grade.salary_grade,
      ...Object.fromEntries(
        STEPS.map(s => [
          `step${s}`,
          Number(grade.steps[s] ?? 0).toLocaleString(undefined, {
  ...
  // Convert the existing steps into the rawData format (numbers, not formatted)
  const buildEditRawData = (version: VersionWithSteps) => {
    const matrix = getMatrix(version);
    return matrix.map(grade => ({
      salary_grade: grade.salary_grade,
      ...Object.fromEntries(STEPS.map(s => [`step${s}`, grade.steps[s] ?? 0])),
    }));
  };

  const handleSaveEdit = async (data: any[]) => {
    if (!editingVersion) return;
    if (!editFields.lbc_reference) throw new Error("Please enter LBC Reference");
    const steps = data.flatMap(row =>
      STEPS.map(s => ({ salary_grade: row.salary_grade, step: s, salary: row[`step${s}`] }))
    );
    await API.put(`/salary-standard-versions/${editingVersion.salary_standard_version_id}/with-steps`, {
      lbc_reference: editFields.lbc_reference,
      tranche: editFields.tranche,
      income_class: editFields.income_class,
      effective_year: editFields.effective_date.toISOString().split("T")[0],
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-gray-600">LBC Reference</label>
          <Input
            value={editFields.lbc_reference}
            onChange={e => setEditFields(p => ({ ...p, lbc_reference: e.target.value }))}
            className="h-8 text-xs"
            placeholder="e.g. LBC No. 111"
          />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-gray-600">Tranche</label>
          <Select value={editFields.tranche} onValueChange={v => setEditFields(p => ({ ...p, tranche: v }))}>
            <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>
              {["1st Tranche","2nd Tranche","3rd Tranche","4th Tranche"].map(t =>
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-gray-600">Income Class</label>
          <Select value={editFields.income_class} onValueChange={v => setEditFields(p => ({ ...p, income_class: v }))}>
            <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Select class" /></SelectTrigger>
            <SelectContent>
              {["1st Class","2nd Class","3rd Class","4th Class"].map(c =>
        ...
        <div className="space-y-1.5">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="w-full h-8 justify-start text-left font-normal text-xs border-gray-200">
                <CalendarIcon className="mr-2 h-3.5 w-3.5 text-gray-400" />
                {editFields.effective_date.toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0">
              <Calendar
                mode="single"
                selected={editFields.effective_date}
                onSelect={d => d && setEditFields(p => ({ ...p, effective_date: d }))}
                initialFocus
              />
            </PopoverContent>
          </Popover>
        ...
        <div className="col-span-2 flex items-center gap-2.5">
          <Checkbox
            ...
            onCheckedChange={c => setEditFields(p => ({ ...p, is_active: c === true }))}
          />
          <label htmlFor="edit_is_active" className="text-xs text-gray-600 cursor-pointer">
  ...
  const transformSalaryPreview = (data: any[]) =>
    data.map(row => ({
      ...
      ...Object.fromEntries(STEPS.map(s => [
        `step${s}`,
        Number(row[`step${s}`]).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
      ])),
    }));
  ...
  return (
    <div className="p-6 w-full space-y-5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!selectedVersion.is_active && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleActivate(selectedVersion.salary_standard_version_id)}
                      disabled={activating}
                      className="gap-1.5 text-xs h-8 border-gray-200 text-gray-600 hover:text-gray-900"
                    >
                      {activating
                        ? <><span className="w-3 h-3 border-2 border-gray-300 border-t-gray-600 rounded-full animate-spin" />Activating…</>
                        : "Set as Active"
                      }
                    </Button>
                  )}
      ...
      {/* Edit modal — pre-populated with existing version data */}
      {editingVersion && (
        <ExcelUploadModal
          isOpen={editOpen}
          onClose={closeEdit}
          title={`Edit Tranche — ${editingVersion.lbc_reference}`}
          entityName="salary matrix"
          confirmationTitle="Confirm Edit"
          confirmationDescription="The existing salary steps for this version will be replaced with the data shown. This cannot be undone."
          // No file parsing needed — data comes pre-loaded
          parseRow={parseSalaryRow}
          requiredColumns={[]}
          onSave={handleSaveEdit}
          additionalFields={renderEditFields()}
          headerRowsToSkip={2}
          customColumns={salaryColumns}
          transformPreview={transformSalaryPreview}
          // Pre-load the existing matrix so the preview table and Save button
          // are immediately active without requiring a new file upload.
          initialRawData={buildEditRawData(editingVersion)}
          initialPreviewData={buildEditPreviewData(editingVersion)}
        />
      )}
    </div>
  );
