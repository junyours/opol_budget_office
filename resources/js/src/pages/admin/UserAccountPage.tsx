    ...
    return (
      <div className="flex items-center gap-2">
        <img
          ...
          className="w-5 h-5 rounded object-contain shrink-0 border border-gray-100"
        />
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-5 h-5 rounded bg-gray-100 border border-gray-200 flex items-center justify-center shrink-0">
        <span className="text-[7px] font-bold text-gray-400 leading-none">{initials}</span>
      </div>
      <span className="text-gray-600 text-[11px]">
        {dept.dept_abbreviation || dept.dept_name}
      </span>
...
const SortIcon = ({ field, active, dir }: { field: SortField; active: SortField; dir: SortDir }) => (
...
//   };
  ...
  const handleSubmit = async () => {
    ...
    if (form.role === 'department-head' && (!form.dept_id || form.dept_id === 'none'))
  ...
  const handleToggleActive = async () => {
    ...
    setToggling(true);
    try {
      await API.put(`/users/${toggleTarget.user_id}`, { is_active: !toggleTarget.is_active });
      toast.success(
    ...
    <div className="p-6 relative">
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        <span className="inline-flex items-center gap-1.5 text-[11px] text-gray-500 bg-gray-100 rounded-full px-2.5 py-1">
          <span className="font-semibold text-gray-700">{counts.total}</span> total
        </span>
        <span className="inline-flex items-center gap-1.5 text-[11px] text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-full px-2.5 py-1">
          ...
          <span className="font-semibold">{counts.active}</span> active
        </span>
        {counts.inactive > 0 && (
          <span className="inline-flex items-center gap-1.5 text-[11px] text-red-600 bg-red-50 border border-red-100 rounded-full px-2.5 py-1">
            <span className="w-1.5 h-1.5 rounded-full bg-red-400" />
            <span className="font-semibold">{counts.inactive}</span> inactive
          </span>
        )}
      </div>
      ...
      <div className="mb-4">
        <span className="text-[10px] text-gray-400 italic hidden sm:block">
          Click a row to edit, change password, or toggle status
        </span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        {processed.length === 0 ? (
          <div className="text-center py-14 text-gray-400 text-sm">
            {isSearching ? (
              <>No users match <span className="font-medium text-gray-600">"{debouncedSearch}"</span>.{' '}
                <button onClick={() => setSearchRaw('')} className="text-gray-600 underline underline-offset-2 font-medium hover:text-gray-900">Clear search</button>
              </>
            ) : isFiltered ? (
              <>No users match the selected filters.{' '}
                <button onClick={() => { setStatusFilter('all'); setRoleFilter('all'); }} className="text-gray-600 underline underline-offset-2 font-medium hover:text-gray-900">Clear filters</button>
              </>
            ) : (
              <>No users yet.{' '}
            ...
            <table className="w-full text-[12px] border-collapse">
              <thead>
                <tr>
                  ...
                  <th
                    className="border-b border-gray-200 bg-white px-4 py-2.5 text-left font-semibold text-gray-600 text-[11px] uppercase tracking-wide cursor-pointer select-none hover:bg-gray-50 transition-colors"
                  ...
                  >
                    Role <SortIcon field="role" active={sortField} dir={sortDir} />
                  </th>
                ...
                {paginated.map(u => {
                  ...
                  return (
                    <tr
                      ...
                      onContextMenu={e => { e.preventDefault(); handleRowClick(e, u); }}
                      className={cn(
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <Avatar className="h-7 w-7 rounded-full border border-gray-100">
                      ...
                      {/* Username */}
                      <td className="px-4 py-3 text-gray-500 font-mono text-[11px]">{u.username}</td>
            ...
            {totalPages > 1 && (
              <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
                <p className="text-[11px] text-gray-400">
                  ...
                  of <span className="font-medium text-gray-600">{processed.length}</span>
                </p>
      ...
      {ctxMenu && (
        <div
          ...
          className="bg-white border border-gray-200 rounded-xl shadow-xl py-1.5 min-w-[180px] overflow-hidden"
        >
          ...
          <div className="px-6 py-5 max-h-[75vh] overflow-y-auto">
            <div className="grid grid-cols-2 gap-x-6 gap-y-4">
              ...
              <div className="space-y-4">
                {([['fname', 'First Name'], ['mname', 'Middle Name'], ['lname', 'Last Name'], ['username', 'Username']] as const).map(([key, label]) => (
                  <div key={key} className="space-y-1.5">
                    <Label className="text-xs font-semibold text-gray-600">
                      {label}
                      {key !== 'mname' && <span className="text-red-400 ml-0.5">*</span>}
                    </Label>
                    <Input
                      value={(form as any)[key]}
                      onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))}
                      className={cn('h-9 text-sm', key === 'username' && 'font-mono', (formErrors as any)[key] && 'border-red-400')}
                    />
                    {(formErrors as any)[key] && <p className="text-[11px] text-red-500">{(formErrors as any)[key]}</p>}
                  </div>
                ))}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={form.dept_id} onValueChange={v => setForm(p => ({ ...p, dept_id: v }))}>
        ...
        <AlertDialogContent className="rounded-2xl max-w-sm border-gray-200">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-sm text-gray-500">
              {toggleTarget?.is_active ? (
                <><span className="font-medium text-gray-700">{toggleTarget?.fname} {toggleTarget?.lname}</span> will be deactivated and <span className="font-medium text-gray-700">will no longer be able to log in</span>.</>
              ) : (
                <><span className="font-medium text-gray-700">{toggleTarget?.fname} {toggleTarget?.lname}</span> will be re-activated and can log in again.</>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction asChild>
              <Button size="sm"
                className={cn('h-8 text-xs', toggleTarget?.is_active ? 'bg-amber-600 hover:bg-amber-700 text-white' : 'bg-emerald-600 hover:bg-emerald-700 text-white')}
      ...
      {/* <AlertDialog open={!!deleteTarget} onOpenChange={o => { if (!o) setDeleteTarget(null); }}>
        <AlertDialogContent className="rounded-2xl max-w-sm border-gray-200">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-sm text-gray-500">
              <span className="font-medium text-gray-700">{deleteTarget?.fname} {deleteTarget?.lname}</span> will be permanently removed. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel asChild>
