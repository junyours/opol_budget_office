...
export default function ProfilePage() {
  ...
  // temporarily while uploading; once the server confirms, we switch to the
  // real /storage/ URL so it survives remounts and page refreshes.
  ...
  // Sync form + avatar when AuthContext fetches fresh user data on mount
  useEffect(() => {
    if (!user) return;
    ...
    // Only update preview from server URL — never overwrite an active blob preview
    // (avatarLoading guard prevents a race where the user effect fires mid-upload)
    if (!avatarLoading) {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Server returns: { data: { avatar: "avatars/xxx.jpg", avatar_url: "http://…" } }
      // FIX: prefer avatar_url from the server if present; otherwise build it ourselves
      // from the relative path. This is the canonical URL and will always work after
      // a page refresh — unlike the local blob URL which is session-only.
      const newAvatarPath = data.data?.avatar ?? null;
      const serverUrl     = data.data?.avatar_url ?? null;
      const resolvedUrl   = serverUrl ?? getAvatarUrl(newAvatarPath);

      // FIX: replace the blob preview with the real /storage/ URL immediately.
      // This way the displayed avatar matches what the server actually stored.
      revokeBlobUrl();
              ...
              <div className="w-24 h-24 rounded-full overflow-hidden ring-2 ring-zinc-200 bg-zinc-100 flex items-center justify-center">
                {avatarPreview ? (
                  <img
                    ...
                    // FIX: if the /storage/ URL fails (e.g. symlink not set up),
                    // fall back to initials gracefully instead of a broken image.
